...
class ChunkCache:
    def __init__(
        self,
        root: str | Path | None,
        chunk_spec: dict[str, Any] | ChunkSpec | None,
        ...
        if isinstance(chunk_spec, ChunkSpec):
            self.chunk_spec = chunk_spec.with_axis_values(
                axis_values,
                axis_order=axis_order,
            )
        else:
            self.chunk_spec = ChunkSpec.from_axis_values(
                chunk_spec,
                axis_values,
                axis_order=axis_order,
            )
    ...
    def _check_exclusive(self) -> None:
        ...
        if self.exclusive:
            ...
            normalized_chunk_spec = self.chunk_spec.to_metadata()
            if meta_chunk_spec != normalized_chunk_spec:
                raise ValueError(
                    "Cache with same cache_id already exists with different chunk_spec. "
                    f"Existing chunk_spec={meta_chunk_spec}, requested={normalized_chunk_spec}. "
                    "Use a different cache_id, or set exclusive=False."
                )
    ...
    def write_metadata(self) -> Path:
        ...
        payload = {
            ...
            "chunk_spec": self.chunk_spec.to_metadata(),
            "version": self.version,
            "axis_order": self.axis_order,
        }
    ...
    def _ordered_axis_values(
        ...
        for axis in axis_order:
            ...
            size = self.chunk_spec.for_axis(axis).size
    ...
    def _axis_chunk_maps(
        ...
        for axis in axis_order:
            ...
            for value in axis_values.get(axis, []):
                chunk_id, _ = self.chunk_spec.for_axis(axis).locate(value)
                value_to_chunk_id[value] = chunk_id
    ...
    def _get_all_axis_values(self, axis: str) -> list[Any]:
        ...
        return self.chunk_spec.for_axis(axis).axis_values()

    def _chunk_spec_size_map(self) -> dict[str, int]:
        return {axis: self.chunk_spec.for_axis(axis).size for axis in self.chunk_spec}
    ...
    def extend_axis_values(
        ...
        *,
        ...
        chunk_sizes = self._chunk_spec_size_map()
        ...
        if missing_axes:
            ...
            for axis in missing_axes:
                ...
                if axis in spec_override:
                    axis_spec = spec_override[axis]
                    if isinstance(axis_spec, Mapping):
                        size = axis_spec.get("size")
                        if size is None:
                            raise KeyError(f"Missing size for axis '{axis}'")
                        chunk_sizes[axis] = int(size)
                    else:
                        chunk_sizes[axis] = int(axis_spec)
                else:
                    chunk_sizes[axis] = len(values) if values else 1
                updated = True
            if self.axis_order is not None:
        ...
        if updated:
            self.chunk_spec = ChunkSpec.from_axis_values(
                chunk_sizes,
                cast(dict[str, list[Any]], self._axis_values),
                axis_order=self.axis_order,
            )
            self._axis_values_serializable = self._make_axis_values_serializable()
    ...
    def _build_chunk_keys(
        ...
        axis_order = self._resolve_axis_order(dict(axis_values))
        axis_chunks = [self.chunk_spec.for_axis(axis).chunks for axis in axis_order]
    ...
    def _build_chunk_plan_for_axes(
        ...
        for axis, requested_values, size in self._ordered_axis_requests(
            ...
            for chunk_id, chunk in chunk_map.items():
                chunk["values"] = self.chunk_spec.for_axis(axis).chunks[chunk_id]
    ...
    def _resolve_axis_chunk_size(self, axis: str) -> int:
        return self.chunk_spec.for_axis(axis).size
    ...
    def cache_is_compatible(
        ...
        if chunk_spec is not None:
            ...
            expected_spec = (
                chunk_spec.to_metadata()
                if isinstance(chunk_spec, ChunkSpec)
                else chunk_spec
            )
            if stable_serialize(meta_spec) != stable_serialize(expected_spec):
                return False
