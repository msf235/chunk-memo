...
chunk-memo provides chunked memoization for grid-style experiments. You define
a parameter grid (axis values), the library partitions it into reusable memo
...
memo = ChunkCache( root="./memo_cache", chunk_spec={"strat": 1, "s": 3},
axis_values=axis_values, collate_fn=collate_fn, ) memo.set_params(params)
...
- `chunk_memo/runner utilities and planning helpers __init__.py`: re-exports   .
