...
- Split spec: a dictionary of lists defining the grid, e.g.
  `{ "strat": ["a", "b"], "s": [1, 2, 3] }`.
...
- `chunk_memo/bridge.py`: removed (parallel runners live in `chunk_memo.runners`).
...
- If the value is a non-sequence iterable, values are materialized and sorted
  by a stable serialization of each value. This makes order deterministic but
  may differ from the original iteration order.
...
The cache should already represent the desired axis subset (use
...
- Missing items are executed via `map_fn` (defaults to a
  `ProcessPoolExecutor`).
