...
collate_fn=collate_fn, ) memo.set_params(params)
...
- `chunk_memo/(parallel runners live in `chunk_memo.runners`) __init__.py`:    .
- `chunk_memo/re-exports public APIs, including `auto_load()`                  .
...
specified (or default) `cache_chunk_spec`. - If `axis_values` is provided
and `allow_superset=True`: finds an exact match or finds a superset cache
that contains all requested data. - If `axis_values` is not provided: finds
...
`auto_load` handles different `cache_chunk_spec` values for the same data. -
Subset detection: use `allow_superset=True` to find and reuse existing caches
that are supersets of your requested data. This allows you to: - Define reduced
...
- If `cache_chunk_spec` is omitted, each axis defaults to a chunk size of 1.
For list/tuple axes this is overridden to the full axis length; other iterables
still default to 1.
...
cache_root="./memo_cache", cache_chunk_spec={"strat": 1, "s": 3},
axis_values={"strat": ["a", "b"], "s": [1, 2, 3]}, collate_fn=collate_fn,
) params = {"alpha": 0.4} broad_cache.set_params(params) output1, diag1 =
run(broad_cache, exec_fn)
